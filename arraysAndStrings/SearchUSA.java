...
public class SearchUSA {
	...
	public static String astar(City source, City destination) {
		ArrayList<ArrayList<Object>> heuristic_map = new ArrayList<>();
		ArrayList<Object> heuristic = new ArrayList<>();
		...
		while (!heuristic_map.isEmpty()) {
			...
			ArrayList<Object> min_path = heuristic_map.remove(min_value);
			...
			for (Map.Entry<String, Integer> entry : node.adjacent_cities.entrySet()) {
				if (road_map.get(entry.getKey()).status == "unvisited") {
					...
					heuristic = new ArrayList<Object>();
